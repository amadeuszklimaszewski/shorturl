...
class UrlService:
    ...
    def get_full_shortened_url(self, short_url: uuid.UUID):
        return f"http://{settings.HOST}:{settings.PORT}/{short_url}"

    async def create_shortened_url(self, url: str) -> ShortenedUrl:
    ...
    async def get_original_url(self, shortened_url: uuid.UUID) -> ShortenedUrl:
